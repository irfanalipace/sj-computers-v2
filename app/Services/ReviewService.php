...
class ReviewService
{
    ...
    {
        $with = ['productMedia:id,product_review_id,media_type,file_path','user:id,name'];
        $review = ProductReview::where('product_id',$product_id)->with('productMedia:id,product_review_id,media_type,file_path','user:id,name,profile_pic,avatar')->select('id','user_id','product_id','title','body','rating')->firstOrFail();
