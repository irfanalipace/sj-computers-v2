      ...
      <div
        ...
        {IS_CHRISTMAS_HOLIDAYS ? (
          ...
          <>
            ...
            <div className=' card-dev-section-paragrap-product'>
              <span className='dilvery-text-paragraph-card'>
                <button style={{ color: '#2c8a9a' }}>FREE Delivery</button>{' '}
                <span style={{ fontWeight: 'bold' }}>
                  {orderEstimatedDelivery?.free_shipment_amount?.estimate_day}
                </span>
                <br></br>
            ...
            <div className='card-dev-section-paragrap-product hidden-on-desktop'>
              <span className='dilvery-text-paragraph-card'>
                <button style={{ color: '#000', fontWeight: 400 }}>
                  Delivery by
                </button>{' '}
                <span style={{ fontWeight: 'bold' }}>
                  {orderEstimatedDelivery?.free_shipment_amount?.estimate_day}
        ...
        <div className='color-card-dev'>
          <button
            ...
            {currentState?.name ? ' ' + currentState?.name : ' Location'}
          </button>
          {show && (
            <LocationModal isOpen={show} handleClose={() => setShow(false)} />
          )}
          {cartItem?.id ? (
            <p className='item-card-add-text-details'>Item already in cart</p>
          ) : (
            <AddToCartAndWarranty isMobile={isMobile} product={product} />
          )}
        </div>
