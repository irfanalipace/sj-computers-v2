...
const ReviewsDialog = ({open, handleDialogOpen, handleClose, reviewId , imgIndex , ReviewsData}) => {
        ...
        <DialogContent sx={{width: "100%", p: 0,}} >
          ...
          <Grid container p={2} rowGap={1} >
              ...
              <Grid item xs={6} pl={2} container>
                <Grid item xs={12}>Full card</Grid>
                <Grid item xs={12} py={1} >
                  <Typography py={1} variant='body2' fontSize={"small"}>Images</Typography>
                  <div style={{display: "flex"}}>
                    {selectedReview?.images?.map((image, index) => (
                  <Box key={index} width={"59px"} height={"59px"} border={"2px solid orange"} sx={{mr: "10px" ,backgroundImage: `url(${image})`, backgroundSize: "cover" }} ></Box>
                    ))}
