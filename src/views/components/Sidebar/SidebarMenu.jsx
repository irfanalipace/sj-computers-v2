...
export default function SidebarMenu({ openState, toggleSidebar }) {
  ...
  return (
    <div className='sideMenu-wrapper'>
      <button onClick={() => toggleSidebar()} className='sideMenu-close-btn'>
        <FontAwesomeIcon icon={faTimes} />
      </button>
      <div className='sideMenu-inner'>
        <div className='sideMenu-header'>
          {user?.profile_pic == 'null' || !user?.profile_pic ? (
            <img className='me-3 default-image' src={userImg} />
          ) : (
            <img className='me-3' src={user?.profile_pic} />
          )}
          <p className='mb-0'>
            Hello,{' '}
            {isAuthenticated ? user?.name : <Link to={'/login'}>Sign In</Link>}
          </p>
        </div>
        <div className='sideMenu'>
          ...
          <ul className='menu-list'>
            ...
            <li className='d-block'>
              <US title='United States' className='country-flag' />
              <Link className='ms-1' onClick={() => toggleSidebar()}>
                United States
              </Link>
            </li>
            <li>
