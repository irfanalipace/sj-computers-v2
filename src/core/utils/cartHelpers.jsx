...
export const deleteNotLocalCartItem = () => {
  ...
  temp_cart_items?.forEach((item, index) => {
    if (item?.notLocal) {
      ...
      let cartTotal = parseFloat(cartDetails?.total) - cartItems[index]?.price;
...
export const compareLocalCartWithDBCart = (array_1, array_2) => {
  ...
  const missingObjects2 = array_2?.filter(
    obj2 => !obj2.notLocal && !array_1?.some(obj1 => obj2.id === obj1.id),
  );
...
export const setCartItemAfterError = (
  ...
  itemsWithErrors?.forEach(errorItem => {
    let cartItem = cart?.find(_item => _item?.id === errorItem?.product_id);
    if (errorItem?.status) {
      ...
      if (
        ...
        const planPrice = cartItem?.plan?.price * errorItem?.available_quantity;
        cartItem = {
          ...cartItem,
          product: {
            ...cartItem.product,
